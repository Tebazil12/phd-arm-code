...
y_gplvm_input = [y_train1;y_train2];

x_gplvm_input = [ [x_real(:,1);x_real(:,1)] [zeros(size(x_real(:,1)));1*ones(size(x_real(:,1)))] ];
...
%     x_shift = actual_x_min + x
    ...
    for test_tap_num = 1:11
        init_latent_vars = [0.00000000001];

        [par, fval, flag] = fminunc(@(opt_pars)gplvm_max_log_like(sigma_f, l, sigma_n_y,...
                                                                  [y_gplvm_input; [y_test(test_tap_num,:)]] , [x_gplvm_input; test_tap_num opt_pars(1) ]),...
        ...
        x_error(set_num-1, test_tap_num,:) = par - (test_tap_num - 10);
        n_flags = n_flags +1;
        fprintf('.');
    end
    fprintf('\n')
